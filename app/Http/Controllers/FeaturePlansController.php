...
class FeaturePlansController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($user->type == 1 && ! $user->can('FeaturePlan-Read')) {
    ...
    public function store(Request $request)
    {
        ...
        $user = auth('sanctum')->user();
        if ($user->type == 1 && ! $user->can('FeaturePlan-Create')) {
            return response()->json([
                'status' => false,
                'message' => 'You do not have permission',
            ], 403);
    ...
    {
        $user = auth('sanctum')->user();
        if ($user->type == 1 && ! $user->can('FeaturePlan-Show')) {
            return response()->json([
                'status' => false,
                'message' => 'You do not have permission',
            ], 403);
    ...
    {
        $user = auth('sanctum')->user();
        if ($user->type == 1 && ! $user->can('FeaturePlan-Update')) {
            return response()->json([
                'status' => false,
                'message' => 'You do not have permission',
            ], 403);
