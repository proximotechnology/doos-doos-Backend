...
class RepresentativeController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->type == 1 && ! $user->can('Read-Representatives')) {
    ...
    {
        $user = auth('sanctum')->user();
        if ($user->type == 1 && ! $user->can('Create-Representative')) {
            return response()->json([
                'status' => false,
                'message' => 'You do not have permission',
            ], 403);
    ...
    {
        $user = auth('sanctum')->user();
        if ($user->type == 1 && ! $user->can('Show-Representative')) {
            return response()->json([
                'status' => false,
                'message' => 'You do not have permission',
            ], 403);
    ...
    {
        $user = auth('sanctum')->user();
        if ($user->type == 1 && ! $user->can('Update-Representative')) {
            return response()->json([
                'status' => false,
                'message' => 'You do not have permission',
            ], 403);
    ...
    {
        $user = auth('sanctum')->user();
        if ($user->type == 1 && ! $user->can('Delete-Representative')) {
            return response()->json([
                'status' => false,
                'message' => 'You do not have permission',
            ], 403);
